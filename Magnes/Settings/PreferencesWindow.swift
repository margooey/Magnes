...
/// Hosts the app preferences UI so it can be summoned from the menu bar.
...
private final class PreferencesViewController: NSViewController {
    ...
    private lazy var frictionSlider: NSSlider = {
        let slider = NSSlider(value: settings.momentumFriction, minValue: 0.80, maxValue: 0.99, target: self, action: #selector(frictionChanged(_:)))
    ...
    private lazy var sensitivitySlider: NSSlider = {
        let slider = NSSlider(value: settings.pointerSensitivity, minValue: 0.18, maxValue: 0.70, target: self, action: #selector(sensitivityChanged(_:)))
...
extension Notification.Name {
    static let settingsDidChange = Notification.Name("com.magnes.settingsDidChange")
}
